...
const informations = reactive(informationList);
//data
...
const confirm = () => { // 点击了确认按钮
    // console.log('length ' + buildingList.value.length)
    for(var i = 0;i < buildingList.value.length;i ++) {
        if(buildingList.value[i].id === currentActiveBuildingId) {
            position = buildingList.value[i].position
        }
    }
}
...
const markerClicked = (id) => {
    ...
    info.id = id;
    info.time = informations[id].time
    info.ChineseName = informations[id].ChineseName
    info.EnglishName = informations[id].EnglishName
    switch(informations[id].type) {
        case '2': info.type = '教学楼';break
        case '3': info.type = '实验楼';break
        case '4': info.type = '宿舍楼';break
        case '5': info.type = '功能区';break
        case '6': info.type = '景观';break
    }
    console.log(info)
}
            ...
            <template v-for="i of campus">
                <Transition name="board-in-out">
                    <single-map class="map" v-if="!i.status" :size="i.map.size" :img-url="i.map.imgUrl" :zoom="3"
                        :markers="i.map.markers" @marker-click="markerClicked" :position = position></single-map>
                </Transition>
            </template>
        </div>
